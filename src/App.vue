    ...
    for (let i = 1; i <= 6; i++) {
      ...
      if (match.status === 'finished' || match.status === 'live') {
        ...
        groups[gKey].matches.push({
          ...
          scoreHome: hasScore ? match.home_score : (match.status === 'finished' ? '?' : null),
          scoreAway: hasScore ? match.away_score : (match.status === 'finished' ? '?' : null),
          played: hasScore || match.status === 'finished',
