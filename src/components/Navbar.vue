    ...
    <div class="max-w-7xl mx-auto px-4 py-2 md:py-3 flex flex-col md:flex-row items-center justify-between gap-3 md:gap-4">
      ...
      <div class="flex items-center justify-between w-full md:w-auto">
        ...
        <div class="flex md:hidden items-center gap-2">
          ...
          <button @click="$emit('logout')" class="p-1.5 bg-red-950/40 border border-red-900/50 text-red-400 rounded-lg text-xs font-semibold">
            Salir
          </button>
        </div>
      </div>
      ...
      <nav class="flex items-center bg-slate-800/60 dark:bg-slate-900/60 p-0.5 md:p-1 rounded-xl border border-slate-700/50 overflow-x-auto max-w-full">
        <button
          ...
          class="px-3 py-1.5 md:px-4 md:py-2 text-xs md:text-sm font-semibold rounded-lg transition-all duration-200 shrink-0"
      ...
      <div class="hidden md:flex items-center gap-4">
        ...
        <div class="flex items-center gap-2">
          <button @click="$emit('toggle-theme')" class="p-1.5 md:p-2 bg-slate-800 hover:bg-slate-700 rounded-lg border border-slate-700 transition duration-200">
            {{ isDark ? '🌞' : '🌙' }}
          </button>
          <button @click="$emit('logout')" class="px-3 py-1.5 bg-red-950/40 hover:bg-red-900/60 text-red-400 rounded-lg text-xs font-semibold border border-red-900/50 transition duration-200">
            Cerrar Sesión
          </button>
        </div>
      </div>
...
const emit = defineEmits(['change-tab', 'logout', 'toggle-theme'])
